...
from game_code.strategy import Offensive, Defensive, Endurable, Hybrid
...
class EnemySelectScene(Scene):
    ...
    def start(self, screen):
        if not self.already_loaded:
            ...
            self.random_diff_btn = ToggleButton(pygame.Rect(width * 1 / 6 - 75, 100, 150, 30), "Random", btn_options)
            self.offence_btn = ToggleButton(pygame.Rect(width * 2 / 6 - 75, 100, 150, 30), "Offense", btn_options)
            self.defence_btn = ToggleButton(pygame.Rect(width * 3 / 6 - 75, 100, 150, 30), "Defense", btn_options)
            self.endurance_btn = ToggleButton(pygame.Rect(width * 4 / 6 - 75, 100, 150, 30), "Endurance", btn_options)
            self.hybrid_btn = ToggleButton(pygame.Rect(width * 5 / 6 - 75, 100, 150, 30), "Hybrid", btn_options)
            toggle_group = [self.random_diff_btn, self.offence_btn, self.defence_btn, self.endurance_btn, self.hybrid_btn]
            for elt in toggle_group:
                elt.set_group(toggle_group)
            self.offence_btn.toggle()
    ...
    def update(self, event):
        for elt in (self.back_btn, self.left_btn, self.right_btn, self.select_btn, self.random_diff_btn,
                    self.offence_btn, self.defence_btn, self.endurance_btn, self.hybrid_btn):
            elt.update(event)
        ...
        if self.select_btn.clicked:
            ...
            possible_strategies = [Offensive(), Defensive(), Endurable(), Hybrid()]
            PenData.current_enemy_strategy = \
                possible_strategies[0] if self.offence_btn.toggled else \
                possible_strategies[1] if self.defence_btn.toggled else \
                possible_strategies[2] if self.endurance_btn.toggled else \
                possible_strategies[3] if self.hybrid_btn.toggled else \
    ...
    def draw(self, screen):
        ...
        for elt in (self.header, self.back_btn, self.left_btn, self.right_btn, self.name_text, self.select_btn,
                    self.offence_btn, self.defence_btn, self.endurance_btn, self.random_diff_btn, self.hybrid_btn):
            elt.draw(screen)

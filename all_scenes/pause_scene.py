...
class PauseScene(Scene):
    ...
    def start(self, screen):
        if not self.already_loaded:
            ...
            self.header = Label(pygame.Rect(width / 2 - 75, height * 1/5 + 10, 150, 50), "Game Paused", options={
                Options.BACKGROUND: (20, 61, 89),
                Options.FOREGROUND: (244, 180, 26),
                Options.BORDER_WIDTH: 0,
            })

            self.back_btn = Button(pygame.Rect(width * 1/5 + 10, height * 1/5 + 10, 60, 40), "Back", {
                Options.BORDER_WIDTH: 0,
                Options.BACKGROUND: (20, 61, 89),
                Options.FOREGROUND: (244, 180, 26),
            ...
            self.forfeit_btn = Button(pygame.Rect(width / 2 - 75, height / 2, 150, 40), "Forfeit", {
                Options.BORDER_WIDTH: 0,
                Options.BACKGROUND: (20, 61, 89),
                Options.FOREGROUND: (244, 180, 26),
                Options.HOVERED_BACKGROUND: (10, 30, 45),
        ...
        if self.last_scene_id != Scene.scene_stack[-2]:
            ...
            dark_cover = pygame.Surface(self.underlay_screen.get_size()).convert_alpha(self.underlay_screen)
            dark_cover.fill((0, 0, 0, 0.6 * 255))
            self.underlay_screen.blit(dark_cover, (0, 0))

    def update(self, event):
        for elt in (self.back_btn, self.forfeit_btn, self.settings_btn):
            elt.update(event)

        if self.back_btn.clicked:
            Scene.pop_scene()
        elif self.forfeit_btn.clicked:
            Scene.push_scene(9)
    ...
    def draw(self, screen):
        ...
        for elt in (self.header, self.back_btn, self.forfeit_btn, self.settings_btn):
            elt.draw(screen)
